...
<h1>{{$title}}</h1>
    <form action="{{route('users.post-edit')}}" method="post">
        ...
        <select name="group_id" id="" class="form-control">
            ...
            <option value="{{$item->id}}" {{old('group_id') ==$item->id || $userDetial->group_id==$item->id?'selected':false}}>{{$item->name}}</option>
            @endforeach
            @endif
        </select>
        @error('group_id')
    ...
    <div class="mb-3">
        ...
        <option value="0" {{ old('status') == 0 || $userDetial->status== 0 ? 'selected' : 'false' }}> Chưa kích hoạt</option>
            <option value="1" {{old('status') == 1 || $userDetial->status== 1 ?'selected':false}}>Kích hoạt</option>
        </select>
        @error('status')
        <span style="color:red">{{$message}}</span>
        @enderror
